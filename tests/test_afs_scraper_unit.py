...
class TestAFSScraper(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Load test database
        with open(os.path.join(self.test_data_dir, "afs_scraper_test_database.json"), 'r') as f:
        ...
        with self.subTest(test_case=test_case_name):
            ...
            if 'dates' in expected:
                ...
                if isinstance(actual['date'], str):
                    self.assertIn(actual['date'], expected['dates'], 
    ...
    def test_extract_showtimes_accurately(self):
        ...
        with patch('requests.Session.get') as mock_get:
            ...
            if events:
                ...
                self.assertRegex(event['date'], date_pattern, 
                               f"Date {event['date']} not in YYYY-MM-DD format")
                
                # Time should contain AM or PM
                self.assertTrue('AM' in event['time'] or 'PM' in event['time'], 
    ...
    def test_detect_special_screenings(self):
        ...
        # Test the Irish film which has a Free Member Monday screening
