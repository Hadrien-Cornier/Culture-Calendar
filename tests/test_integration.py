...
class TestScraperIntegration:
    ...
    def test_classical_scraper_initialization(
        self, mock_env_vars, mock_classical_data_file
    ):
    ...
    def test_classical_scraper_data_loading(
        self, mock_env_vars, mock_classical_data_file
    ):
...
class TestMultiVenueIntegration:
    ...
    def test_venue_assignment(self, multi_venue_scraper):
        ...
        with patch.object(
            multi_venue_scraper.afs_scraper, "scrape_events", return_value=test_events
        ):
            with patch.object(
                multi_venue_scraper.hyperreal_scraper, "scrape_events", return_value=[]
            ):
                with patch.object(
                    multi_venue_scraper.symphony_scraper,
                    "scrape_events",
                    return_value=[],
                ):
                    with patch.object(
                        multi_venue_scraper.early_music_scraper,
                        "scrape_events",
                        return_value=[],
                    ):
                        with patch.object(
                            multi_venue_scraper.la_follia_scraper,
                            "scrape_events",
                            return_value=[],
                        ):
                            with patch.object(
                                multi_venue_scraper.alienated_majesty_scraper,
                                "scrape_events",
                                return_value=[],
                            ):
                                with patch.object(
                                    multi_venue_scraper.first_light_scraper,
                                    "scrape_events",
                                    return_value=[],
                                ):
    ...
    def test_error_handling(self, multi_venue_scraper):
        ...
        with patch.object(
            multi_venue_scraper.afs_scraper,
            "scrape_events",
            side_effect=Exception("AFS Error"),
        ):
            with patch.object(
                multi_venue_scraper.hyperreal_scraper,
                "scrape_events",
                return_value=[
                    {"title": "Working Event", "date": "2025-07-15", "time": "7:30 PM"}
                ],
            ):
                with patch.object(
                    multi_venue_scraper.symphony_scraper,
...
class TestProcessorIntegration:
    ...
    def test_process_film_events(self, event_processor, sample_film_event):
        ...
        with patch.object(
            event_processor,
            "_get_ai_rating",
            return_value={"score": 8, "summary": "Great film"},
        ):
    ...
    def test_process_book_club_events(self, event_processor, sample_book_club_event):
        ...
        with patch.object(
            event_processor,
            "_get_book_club_rating",
            return_value={"score": 7, "summary": "Thoughtful discussion"},
        ):
    ...
    def test_process_concert_events(self, event_processor, sample_concert_event):
        ...
        with patch.object(
            ...
            return_value={"score": 9, "summary": "Excellent performance"},
        ):
    ...
    def test_preference_scoring(self, event_processor, sample_film_event):
        ...
        with patch.object(
            event_processor,
            "_get_ai_rating",
            return_value={"score": 6, "summary": "Contains test keywords"},
        ):
            events = [sample_film_event]
            processed = event_processor.process_events(events)
    ...
    def test_scraper_to_processor_pipeline(self, mock_env_vars):
        ...
        with patch("src.processor.SummaryGenerator"):
            ...
            with patch.object(
                processor,
                "_get_ai_rating",
                return_value={"score": 8, "summary": "Test summary"},
            ):
