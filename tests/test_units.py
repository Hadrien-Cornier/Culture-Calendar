...
class TestLLMService:
    ...
    def test_validate_extraction(self, llm_service, sample_film_event):
        ...
        mock_response = Mock()
        mock_response.content = [Mock()]
        mock_response.content[0].text = "VALID: All required fields present and well-formatted"
        llm_service.anthropic.messages.create.return_value = mock_response
...
class TestBaseScraper:
    ...
    def test_requests_tier(self, mock_get, test_scraper, sample_film_event):
        ...
        # Mock LLM extraction
        test_scraper.llm_service.extract_data = Mock(return_value={
            'extracted_events': [sample_film_event]
        })
...
class TestSummaryGenerator:
    ...
    def summary_generator(self, mock_env_vars, temp_cache_file):
        ...
        with patch('src.summary_generator.Anthropic') as mock_anthropic:
            with patch('src.summary_generator.SummaryGenerator._load_cache') as mock_load:
    ...
    def test_generate_summary_new(self, summary_generator, sample_film_event):
        ...
        result = summary_generator.generate_summary(sample_film_event, force_regenerate=True)
    ...
    def test_prompt_building(self, summary_generator, sample_film_event):
        ...
        # Test film prompt
        film_prompt = summary_generator._build_movie_prompt("Test Movie", "Description", sample_film_event)
        assert "Test Movie" in film_prompt
        assert "8-12 words" in film_prompt
        
        # Test concert prompt
        concert_event = {'venue': 'Symphony', 'composers': ['Beethoven']}
        concert_prompt = summary_generator._build_concert_prompt("Concert", "Description", concert_event)
...
class TestScraperGenerator:
    ...
    def test_convenience_function(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('src.scraper_generator.ScraperGenerator.save_scraper') as mock_save:
