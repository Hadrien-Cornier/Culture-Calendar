...
class TestFirstLightScraper:
    ...
    def setup_class(cls):
        ...
        # Load test database
        test_db_path = Path("tests/First_Light_test_data/first_light_scraper_test_database.json")
    ...
    def test_individual_extractions(self):
        ...
        if failed_tests:
            failure_msg = "\n".join([f"Test {test_id}: {error}" for test_id, error in failed_tests])
    ...
    def _test_single_extraction(self, test_case):
        ...
        try:
            if "book-club" in url:
                extracted_events = self.scraper.extract_book_club_events(html_content, url)
            else:
                extracted_events = self.scraper.extract_author_events(html_content, url)
        except AttributeError as e:
            raise AttributeError(f"Scraper missing method for extracting from {url}: {e}")
        ...
        if not matching_event:
            raise AssertionError(f"No matching event found for: {expected_output.get('title')}")
        
        # Compare fields
        self._compare_event_fields(matching_event, expected_output, test_case["test_id"])
