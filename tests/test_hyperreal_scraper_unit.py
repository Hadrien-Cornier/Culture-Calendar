...
class TestHyperrealScraper:
    ...
    def setup_method(self):
        ...
        # Load test database
        with open(self.test_data_dir / "hyperreal_scraper_test_database.json", 'r') as f:
    ...
    def test_individual_event_extraction(self):
        ...
        # Test core fields
        assert result["title"] == expected["title"], f"Title mismatch: {result['title']} != {expected['title']}"
        ...
        assert result["is_special_screening"] == expected["is_special_screening"], f"Special screening flag mismatch"
        
        # Test optional fields
        if expected.get("trailer_url"):
            assert result["trailer_url"] == expected["trailer_url"], f"Trailer URL mismatch"
    ...
    def test_movie_title_extraction(self):
        ...
        test_cases = [
            ...
            ("Sad Girl Cinema ~ HOUSE OF TOLERANCE at HYPERREAL FILM CLUB", "HOUSE OF TOLERANCE"),
            ("First Times ~ PLEASE BABY PLEASE at HYPERREAL FILM CLUB", "PLEASE BABY PLEASE"),
            ("Fangoria presents THE BURNING at HYPERREAL FILM CLUB", "THE BURNING"),
            ("KOKOMO CITY presented by Queertopia at HYPERREAL FILM CLUB", "KOKOMO CITY"),
            ("MISS JUNETEENTH free screening at HYPERREAL FILM CLUB", "MISS JUNETEENTH"),
        ...
        for full_title, expected_title in test_cases:
            ...
            assert result == expected_title, f"Title extraction failed: '{full_title}' -> '{result}' (expected '{expected_title}')"
    
    def test_presenter_extraction(self):
        """Test presenter/series extraction from full event titles."""
        test_cases = [
            ("A Woman of Taste Presents ~ MERMAIDS at HYPERREAL FILM CLUB", "A Woman of Taste"),
            ("Sad Girl Cinema ~ HOUSE OF TOLERANCE at HYPERREAL FILM CLUB", "Sad Girl Cinema"),
            ("First Times ~ PLEASE BABY PLEASE at HYPERREAL FILM CLUB", "First Times"),
            ("Fangoria presents THE BURNING at HYPERREAL FILM CLUB", "Fangoria Presents"),
            ("KOKOMO CITY presented by Queertopia at HYPERREAL FILM CLUB", "Presented By Queertopia"),
        ...
        for full_title, expected_presenter in test_cases:
            ...
            if expected_presenter is None:
                assert result is None, f"Should not extract presenter from '{full_title}', got '{result}'"
            else:
                assert result is not None, f"Should extract presenter from '{full_title}'"
                # Allow for some flexibility in exact string matching
                assert any(part.lower() in result.lower() for part in expected_presenter.lower().split()), \
    ...
    def test_special_screening_detection(self):
        ...
        for title, expected_special in test_cases:
            ...
            assert result == expected_special, f"Special screening detection failed for '{title}': got {result}, expected {expected_special}"
    ...
    def test_calendar_event_link_extraction(self):
        ...
        assert coverage > 0.8, f"Should find at least 80% of expected events. Found {len(intersection)}/{len(expected_urls_set)} events"
        
        # Verify URL format
        for url in event_links:
            assert url.startswith("https://hyperrealfilm.club/events/2025/"), f"Invalid event URL format: {url}"
            assert "movie-screening" in url, f"Event URL should contain 'movie-screening': {url}"
    ...
    def test_calendar_url_generation(self):
        ...
        for year, month, expected_url in test_cases:
            ...
            assert result == expected_url, f"Calendar URL generation failed: got {result}, expected {expected_url}"
    ...
    def test_schema_structure(self):
        ...
        expected_fields = [
            "title", "full_title", "presenter", "dates", "times", 
            "end_times", "venue", "description", "trailer_url", 
            "url", "is_special_screening"
        ]
        
        # Schema should be a dictionary or have attributes for these fields
        for field in expected_fields:
            assert hasattr(schema, field) or field in schema, f"Schema missing field: {field}"
