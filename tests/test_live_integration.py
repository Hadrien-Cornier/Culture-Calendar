...
class LiveIntegrationTestFramework:
    ...
    def validate_live_event(self, event: Dict, venue_name: str) -> Dict[str, Any]:
        ...
        # Schema validation
        schema_validation = SchemaRegistry.validate_event_data(event, venue_name.lower())
        ...
        if 'date' in event and event['date']:
            try:
                ...
                if event_date < today:
                    validation_result['warnings'].append(f"Event date {event['date']} is in the past")
                
                # Check if date is too far in the future (more than 1 year)
                if event_date > today + timedelta(days=365):
                    validation_result['warnings'].append(f"Event date {event['date']} is more than 1 year in the future")
                    
            except ValueError:
                validation_result['errors'].append(f"Invalid date format: {event['date']}")
        ...
        if 'time' in event and event['time']:
            ...
            if not re.match(time_pattern, event['time'], re.IGNORECASE):
                validation_result['warnings'].append(f"Time format may be non-standard: {event['time']}")
        
        # URL validation
        if 'url' in event and event['url']:
            if not event['url'].startswith(('http://', 'https://')):
                validation_result['warnings'].append(f"URL may be malformed: {event['url']}")
...
class TestLiveScraperIntegration:
    ...
    def test_all_venue_scrapers_live(self):
        ...
        for venue, result in results.items():
            ...
            print(f"{status} {venue}: {result['events_found']} events, {result['valid_events']} valid")
            if not result['success'] and 'error' in result:
                print(f"    Error: {result['error']}")
        
        # Assertions
        assert successful_venues >= 3, f"At least 3 venues should work, but only {successful_venues} succeeded"
        assert total_valid_events >= 5, f"Should find at least 5 valid events, but found {total_valid_events}"
...
class TestLiveProcessingPipeline:
    ...
    def test_live_scraping_to_processing(self):
        ...
        assert len(scraped_events) >= 5, f"Should scrape at least 5 events, but got {len(scraped_events)}"
        assert len(events_by_venue) >= 3, f"Should get events from at least 3 venues, but got {len(events_by_venue)}"
        ...
        if scraped_events:
            ...
            for event in processed_events:
                ...
                assert 'final_rating' in event, "Processed event should have final rating"
                assert 'rating_explanation' in event, "Processed event should have rating explanation"
                assert 1 <= event['final_rating'] <= 10, f"Rating should be 1-10, got {event['final_rating']}"
            ...
            if processed_events:
                ...
                for event in processed_events[:2]:  # Generate summaries for first 2 events
    ...
    def test_live_calendar_generation(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.ics', delete=False) as temp_file:
            try:
                ...
                assert os.path.getsize(temp_file.name) > 0, "Calendar file should not be empty"
                ...
                for element in required_elements:
                    assert element in calendar_content, f"Calendar should contain {element}"
                
                # Check that event titles are in calendar
                for event in sample_events:
                    if event.get('title'):
                        assert event['title'] in calendar_content, f"Event title should be in calendar: {event['title']}"
                
                print(f"✅ Calendar generated successfully with {len(sample_events)} events")
...
def test_live_mini_refresh():
    ...
    try:
        ...
        results['venues_tested'] = len(set(event.get('venue', 'Unknown') for event in scraped_events))
        
        print(f"Scraped {len(scraped_events)} events from {results['venues_tested']} venues")
        ...
        if valid_events:
            ...
            if processed_events:
                ...
                for event in processed_events:
                    ...
                    event['rating_explanation'] = event.get('rating_explanation', 'Live test')
        ...
        # Assertions
        assert results['venues_tested'] >= 3, f"Should test at least 3 venues, but tested {results['venues_tested']}"
        assert results['total_events'] >= 5, f"Should find at least 5 events, but found {results['total_events']}"
        assert results['valid_events'] >= 3, f"Should have at least 3 valid events, but had {results['valid_events']}"
