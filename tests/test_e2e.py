...
class TestCompleteWorkflow:
    ...
    def mock_complete_system(self, mock_env_vars, temp_classical_data_file):
        ...
        # Mock AI responses
        mock_ai_responses = {
            'film_rating': {'score': 8, 'summary': 'Compelling drama with excellent cinematography'},
            'book_rating': {'score': 7, 'summary': 'Thoughtful exploration of human nature'},
            'concert_rating': {'score': 9, 'summary': 'Masterful interpretation of classical works'},
    ...
    def test_full_scraping_pipeline(self, mock_complete_system):
        ...
        with patch.object(scraper.afs_scraper, 'scrape_events', return_value=[mock_complete_system['events'][0]]):
            with patch.object(scraper.first_light_scraper, 'scrape_events', return_value=[mock_complete_system['events'][1]]):
                with patch.object(scraper.symphony_scraper, 'scrape_events', return_value=[mock_complete_system['events'][2]]):
                    with patch.object(scraper.hyperreal_scraper, 'scrape_events', return_value=[]):
                        with patch.object(scraper.early_music_scraper, 'scrape_events', return_value=[]):
                            with patch.object(scraper.la_follia_scraper, 'scrape_events', return_value=[]):
                                with patch.object(scraper.alienated_majesty_scraper, 'scrape_events', return_value=[]):
                                    # Execute scraping
        ...
        with patch('src.processor.SummaryGenerator'):
            ...
            with patch.object(processor, '_get_ai_rating', side_effect=mock_get_ai_rating):
                with patch.object(processor, '_get_book_club_rating', side_effect=mock_get_book_rating):
                    with patch.object(processor, '_get_classical_rating', side_effect=mock_get_classical_rating):
    ...
    def test_error_handling_in_pipeline(self, mock_complete_system):
        ...
        with patch('src.processor.SummaryGenerator'):
            ...
            with patch.object(processor, '_get_ai_rating', side_effect=mock_failing_ai_rating):
    ...
    def test_rating_filtering_workflow(self, mock_complete_system):
        ...
        for rating_filter, filtered_events in [(8, high_rated), (5, medium_rated)]:
            if filtered_events:
                with tempfile.NamedTemporaryFile(suffix=f'_rating_{rating_filter}.ics', delete=False) as temp_file:
                    try:
        ...
        with tempfile.NamedTemporaryFile(suffix='.ics', delete=False) as temp_file:
            try:
                ...
                for element in required_ics_elements:
                    assert element in ics_content, f"Missing required ICS element: {element}"
...
class TestPerformanceRequirements:
    ...
    def test_processing_time_limits(self, mock_complete_system):
        ...
        with patch('src.processor.SummaryGenerator'):
            ...
            with patch.object(processor, '_get_ai_rating', side_effect=quick_ai_rating):
                ...
                assert processing_time < len(test_events) * 1.0, f"Processing took too long: {processing_time}s"
    ...
    def test_memory_usage(self, mock_complete_system):
        ...
        events = mock_complete_system['events'] * 5  # Duplicate to create larger dataset
        
        with patch('src.processor.SummaryGenerator'):
            processor = EventProcessor()
            
            with patch.object(processor, '_get_ai_rating', return_value={'score': 8, 'summary': 'Test'}):
                ...
                assert final_size < initial_size * 10, f"Memory usage too high: {final_size} vs {initial_size}"
