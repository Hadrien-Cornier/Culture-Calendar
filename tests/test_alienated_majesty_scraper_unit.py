...
class TestAlienatedMajestyBooksScraper(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Load test database
        with open(
            self.test_data_dir / "alienated_majesty_scraper_test_database.json", "r"
        ) as f:
    ...
    def _compare_event(self, actual, expected, event_index):
        ...
        for field, expected_value in expected.items():
            ...
            if actual_value != expected_value:
                errors.append(
                    f"  Event {event_index} - {field}: Expected { expected_value!r}, got {actual_value!r}"
                )
        ...
        for field in actual.keys():
            if field not in expected:
                errors.append(
                    f"  Event {event_index} - Unexpected field {field}: {actual[field]!r}"
                )
    ...
    def test_date_time_parsing(self):
        ...
        for test_case in self.test_database["test_cases"]:
            ...
            print(
                f"  Test {test_case['test_id']}: Dates: {dates_ok}/{total_events} correct, Times: {times_ok}/{total_events} correct"
            )
