...
class TestEventDataQuality:
    ...
    def test_required_fields_validation(self, sample_film_event, sample_book_club_event, sample_concert_event):
        ...
        book_validation = SchemaRegistry.validate_event_data(sample_book_club_event, 'book_club')
        assert book_validation['is_valid'] == True
        assert len(book_validation['errors']) == 0
        
        # Test concert event
        concert_validation = SchemaRegistry.validate_event_data(sample_concert_event, 'concert')
    ...
    def test_time_format_validation(self):
        ...
        for time in valid_times:
            ...
            assert re.match(time_pattern, time, re.IGNORECASE), f"Time {time} doesn't match expected pattern"
    ...
    def test_url_format_validation(self):
        ...
        for url in valid_urls:
            assert url.startswith(('http://', 'https://')), f"URL {url} should have valid protocol"
    ...
    def test_venue_consistency(self):
        ...
        expected_venues = {'AFS', 'Hyperreal', 'Symphony', 'EarlyMusic', 'LaFollia', 'AlienatedMajesty', 'FirstLight'}
        ...
        for event in test_events:
            assert event['venue'] in expected_venues, f"Venue {event['venue']} not in expected list"
    ...
    def test_extraction_hints_completeness(self):
        ...
        for field in important_film_fields:
            if field in film_schema:
                ...
                assert 'extraction_hints' in field_def, f"Film field {field} missing extraction hints"
                assert len(field_def['extraction_hints']) > 0, f"Film field {field} has empty extraction hints"
        ...
        for field in important_book_fields:
            if field in book_schema:
                ...
                assert 'extraction_hints' in field_def, f"Book field {field} missing extraction hints"
                assert len(field_def['extraction_hints']) > 0, f"Book field {field} has empty extraction hints"
        ...
        for field_name, field_def in film_schema.items():
            ...
            for pattern in patterns:
                try:
                    ...
                    assert False, f"Invalid regex pattern in {field_name}: {pattern} - {e}"
    ...
    def test_schema_field_descriptions(self):
        ...
        for schema_type in schemas:
            ...
            for field_name, field_def in schema.items():
                ...
                assert description.strip(), f"Field {field_name} in {schema_type} schema missing description"
                assert len(description) > 10, f"Field {field_name} description too short: {description}"
...
class TestDataConsistency:
    ...
    def test_venue_name_consistency(self):
        ...
        assert expected_venues == mapped_venues, f"Venue mapping inconsistency: {expected_venues - mapped_venues}"
    ...
    def test_event_type_consistency(self):
        ...
        assert expected_types.issubset(available_types), f"Missing event types: {expected_types - available_types}"
    ...
    def test_field_naming_consistency(self):
        ...
        for schema_type in schemas:
            ...
            assert len(missing_universal) == 0, f"Schema {schema_type} missing universal fields: {missing_universal}"
