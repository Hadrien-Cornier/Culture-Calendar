...
class TestEmptyExtractionValidation:
    ...
    def test_validate_empty_extraction_result(self):
        ...
        assert scraper._validate_extraction_result([{"date": "2025-01-01"}]) == False  # Missing title
        
        # Test with list containing valid events
        assert scraper._validate_extraction_result([{"title": "Test Event", "date": "2025-01-01"}]) == True
    ...
    def test_standardize_event_data_with_empty_title(self):
        ...
        result = scraper._standardize_event_data({"date": "2025-01-01"}, "https://example.com")
        assert result is None
        
        # Test with valid title
        result = scraper._standardize_event_data({"title": "Test Event"}, "https://example.com")
    ...
    def test_format_extraction_result_with_empty_data(self):
        ...
        result = scraper._format_extraction_result([{"title": ""}, {"title": None}], "https://example.com")
        assert result == []
        
        # Test with valid data
        result = scraper._format_extraction_result({"title": "Test Event"}, "https://example.com")
        assert len(result) == 1
...
class TestEmptyExtractionIntegration:
    ...
    def test_llm_extraction_returns_invalid_json(self, mock_anthropic):
        ...
        mock_response = Mock()
        mock_response.content = [Mock()]
        mock_response.content[0].text = '{"title": "Test", "date": "2025-01-01",}'  # Invalid JSON
        mock_client.messages.create.return_value = mock_response
        mock_anthropic.return_value = mock_client
...
class TestEmptyExtractionErrorReporting:
    ...
    def test_empty_extraction_error_messages(self):
        ...
        test_cases = [
            ("No JSON found", "I couldn't find any events", "No valid JSON found in response"),
...
class TestEmptyExtractionFailureScenarios:
    ...
    def test_extraction_succeeds_but_produces_null_data(self):
        ...
        assert result['success'] == False  # ✅ Now correctly fails
        assert "No useful data extracted" in result['error']  # ✅ Clear error message
        assert result['data'] == {"title": None, "date": None, "time": None}  # ✅ Includes data for debugging
    ...
    def test_extraction_succeeds_but_produces_empty_strings(self):
        ...
        assert result['success'] == False  # ✅ Now correctly fails
        assert "No useful data extracted" in result['error']  # ✅ Clear error message
        assert result['data'] == {"title": "", "date": "", "time": ""}  # ✅ Includes data for debugging
    ...
    def test_extraction_succeeds_but_missing_required_fields(self):
        ...
        assert result['success'] == False  # ✅ Now correctly fails
        assert "No useful data extracted" in result['error']  # ✅ Clear error message
        assert result['data'] == {"date": "2025-01-01", "time": "7:30 PM"}  # ✅ Includes data for debugging
    ...
    def test_extraction_succeeds_with_valid_data(self):
        ...
        valid_response = '{"title": "Test Event", "date": "2025-01-01", "time": "7:30 PM"}'
        result = llm_service._parse_extraction_response(valid_response, schema)
        # Should succeed with valid data
        assert result['success'] == True  # ✅ Correctly succeeds
        assert result['data'] == {"title": "Test Event", "date": "2025-01-01", "time": "7:30 PM"}
