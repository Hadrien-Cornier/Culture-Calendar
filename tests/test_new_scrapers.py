...
class ScraperTestFramework:
    ...
    def validate_scraper_events(self, scraper, events: List[Dict]) -> Dict[str, Any]:
        ...
        validation_results = {
            ...
            "schema_validation": [],
            "llm_validation": [],
            "summary": {"schema_passed": 0, "llm_passed": 0, "overall_quality": 0.0},
        }
        ...
        if len(events) > 0:
            ...
            if validation_results["llm_validation"]:
                ...
                validation_results["summary"]["overall_quality"] = (
                    schema_score + llm_score
                ) / 2
            else:
    ...
    def _validate_event_schema(
        self, event: Dict, schema: Dict, venue_name: str
    ) -> Dict:
    ...
    def test_scraper_robustness(self, scraper) -> Dict[str, Any]:
        ...
        if target_urls:
            ...
            if fallback_events:
                fallback_validation = self.validate_scraper_events(
                    scraper, fallback_events
                )
                results["fallback_data_quality"] = fallback_validation["summary"][
                    "overall_quality"
                ]
...
def test_new_scraper_architecture(scraper_class, scraper_name):
    ...
    assert hasattr(
        scraper, "get_target_urls"
    ), f"{scraper_name} missing get_target_urls method"
    assert hasattr(
        scraper, "get_data_schema"
    ), f"{scraper_name} missing get_data_schema method"
    assert hasattr(
        scraper, "get_fallback_data"
    ), f"{scraper_name} missing get_fallback_data method"
    ...
    if fallback_events:
        ...
        fallback_validation = test_framework.validate_scraper_events(
            scraper, fallback_events
        )
        print(
            f"Fallback data quality: {fallback_validation['summary']['overall_quality']:.2f}"
        )

        # Fallback data should be high quality
        assert (
            fallback_validation["summary"]["overall_quality"] >= 0.8
...
def test_first_light_scraper_live():
    ...
    print(
        f"Schema validation: {validation_results['summary']['schema_passed']}/{validation_results['total_events']} passed"
    )
    if validation_results["llm_validation"]:
        print(
            f"LLM validation: {validation_results['summary']['llm_passed']}/{validation_results['total_events']} passed"
        )
    print(f"Overall quality: {validation_results['summary']['overall_quality']:.2f}")

    # Quality thresholds
    assert (
        validation_results["summary"]["overall_quality"] >= 0.7
...
def test_llm_validation_system():
    ...
    assert validation_result[
        "is_valid"
    ], f"Good event should be valid: {validation_result['reason']}"
    assert (
        validation_result["confidence"] >= 0.7
    ), f"Confidence too low: {validation_result['confidence']}"
    ...
    assert (
        not bad_validation_result["is_valid"]
        or bad_validation_result["confidence"] < 0.5
...
def test_progressive_fallback_system():
    ...
    if not events:
        ...
        ), "Fallback should provide events when web scraping fails"
...
def test_schema_registry():
    ...
    print(
        f"Schema registry test passed. Available types: {SchemaRegistry.get_available_types()}"
    )
