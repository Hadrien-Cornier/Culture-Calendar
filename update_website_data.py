...
# The venue *code* stamped onto events by ``MultiVenueScraper`` maps back to a
# key under ``venues:`` in ``config/master_config.yaml`` so every event in
# ``docs/data.json`` can carry ``venue_address`` + ``venue_display_name``.
#
# That mapping used to be a hardcoded dict here, duplicated verbatim in
# ``scripts/build_event_shells.py``. Both copies had drifted and were missing
# ``ArtAustin`` and ``IshidaDance``, so those venues shipped with an empty
# address. It now comes from the config, where registry-backed venues derive
# themselves and only the hand-written scrapers need an explicit alias.
...
def _lookup_venue_metadata(
    venue_code: str, venues_config: dict, code_map: dict
) -> tuple[str, str]:
    ...
    host venue instead of the aggregator). We try the code→config-key map
    first, then fall back to the raw code as the display name. Address defaults
    to an empty string when no config entry matches so downstream builders can
    detect the miss.
    """
    if not venue_code:
        return "", ""
    config_key = code_map.get(venue_code)
...
def _enrich_events_with_venue_metadata(events: list, config: ConfigLoader) -> list:
    ...
    code_map = config.get_venue_code_map()
    for event in events:
        display_name, address = _lookup_venue_metadata(
            event.get("venue", ""), venues_config, code_map
        )
