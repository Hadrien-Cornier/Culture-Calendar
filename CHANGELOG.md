...
### task-6.4 — DONE — 2026-04-30T18:28:20Z
- commit: cbe9998
...
### task-6.5 — DONE — 2026-04-30T18:47:19Z
...
### task-7.1 — DONE — 2026-04-30T19:27:53Z
...
### task-7.3 — DONE — 2026-04-30T19:38:00Z
...
### task-6.6b — DONE — 2026-04-30T22:47:16Z
- commit: 5c3441e964ff7e93224b1a6b8af333bf4068ac61
- files: src/scrapers/austin_symphony_scraper.py, src/scrapers/austin_opera_scraper.py, src/scrapers/austin_chamber_music_scraper.py, src/scrapers/early_music_scraper.py, src/scrapers/la_follia_scraper.py, src/scrapers/ballet_austin_scraper.py
- validation: green
- summary: Migrated all six static-JSON venues (Austin Symphony, Austin Opera, Austin Chamber Music, Early Music Austin, La Follia, Ballet Austin) from near-duplicate per-file implementations onto the StaticJsonScraper base class delivered in task-6.6a. Each per-venue file collapses to a thin ~25-line subclass that supplies only the venue's distinct config (base_url, venue_name, top_level_key, default_event_type, default_time, default_location, expand_dates) and points data_file at the right docs/ JSON. Net diff: -377/+78. Public API preserved — constructors still accept ``(config=None, venue_key=...)``, the ``data_file`` attribute is still settable for fixtures (relied on by tests/test_ballet_dance_review.py), and Symphony continues to preserve parallel ``dates``/``times`` arrays via ``expand_dates=False``. Type resolution now flows through the base class's ladder (per-event JSON → master_config.yaml ``assumed_event_category`` → constructor default), so Ballet Austin is pinned to ``type=dance`` at all three tiers. All 873 tests + 25 skipped, including the static-JSON-scraper unit suite (26 tests) and the ballet-dance-review end-to-end suite, pass green.
