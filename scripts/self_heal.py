...
a code fix using Claude, applies it, runs tests, and commits the fix
directly to main - no human merge step required.
...
# ---------------------------------------------------------------------------
# Direct commit (no PR, no manual merge)
# ---------------------------------------------------------------------------


def commit_fix_directly(
    diagnosis: str,
    fix: dict,
    scraper_name: Optional[str],
    issue_number: Optional[int],
) -> Optional[str]:
    """Commit the fix directly to main and push. Returns the commit SHA.

    The fix has already passed the full test suite, so it goes straight
    to main - no PR, no manual merge step.
    """
    # Configure git
    subprocess.run(
    ...
    msg = (
        ...
        f"Auto-generated by the self-healing agent. Tests passed."
    )
    if issue_number:
        msg += f"\nCloses #{issue_number}"
    subprocess.run(
    ...
    # Push to main
    subprocess.run(
        ["git", "push", "origin", "main"], cwd=REPO_ROOT, check=True
    )
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Diagnose only, don't apply fixes or commit",
    )
    ...
    for attempt in range(1, MAX_FIX_ATTEMPTS + 1):
        ...
        if passed:
            print("  Tests passed! Committing fix to main...")
            commit_url = commit_fix_directly(
                result["diagnosis"], result["fix"], scraper_name, issue_number
            )
            if commit_url and args.issue:
                requests.post(
                    f"{GITHUB_API}/repos/{REPO}/issues/{args.issue}/comments",
                    headers=_gh_headers(),
                    json={
                        "body": (
                            ...
                            f"**Fix committed**: {commit_url}\n\n"
                            f"Tests passed. Fix is live on main."
                        )
                    },
                    timeout=15,
                )
