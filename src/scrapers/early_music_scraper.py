"""
Texas Early Music Project scraper using JSON data loading
"""
...
class EarlyMusicAustinScraper(BaseScraper):
    ...
    def __init__(self):
        ...
        self.data_file = "/Users/HCornier/Documents/Github/Culture-Calendar/docs/classical_data.json"
    ...
    def get_data_schema(self) -> Dict:
        ...
        return {
            'title': {'type': 'string', 'required': True, 'description': 'Concert title'},
            'program': {'type': 'string', 'required': False, 'description': 'Concert program'},
            'featured_artist': {'type': 'string', 'required': False, 'description': 'Featured artist or ensemble'},
            'composers': {'type': 'array', 'required': False, 'description': 'List of composer names'},
            'works': {'type': 'array', 'required': False, 'description': 'List of musical works'},
            'series': {'type': 'string', 'required': False, 'description': 'Concert series name'},
    ...
    def scrape_events(self, use_cache: bool = True) -> List[Dict]:
        ...
        try:
            ...
            print(f"Loaded {len(standardized_events)} Early Music events from JSON")
            return standardized_events
            
        except Exception as e:
            print(f"Error loading Early Music events from JSON: {e}")
            return []