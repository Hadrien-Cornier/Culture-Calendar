...
class FirstLightAustinScraper(BaseScraper):
    ...
    def get_target_urls(self) -> List[str]:
        ...
        return [
            ...
            f"{self.base_url}/events"  # Main events page - individual pages will be scraped separately
        ]
    ...
    def parse_book_club_date(self, date_str):
        ...
        try:
            ...
            # Parse the date (assuming future year since these are future events)
            try:
                date_obj = datetime.strptime(f"{future_year} {cleaned_date}", "%Y %A, %B %d")
            except ValueError:
                # If that fails, try without the day of week
                date_obj = datetime.strptime(f"{future_year} {cleaned_date.split(', ')[-1]}", "%Y %B %d")
    ...
    def extract_author_events(self, html_content, url):
        ...
        if story_content:
            ...
            if date_time_elem:
                date_str, time_str = self.parse_date_time(date_time_elem.get_text().strip())
        else:
        ...
        if description_elem:
            ...
            # Look for book titles in description (usually in caps or emphasized)
            if not book:
                # Look for patterns like "THE 5 TYPES OF WEALTH" in all caps
                book_match = re.search(r'THE\s+[A-Z0-9\s]+?(?:\s+will\s+teach|\.|,|\s+was\s+an\s+instant|\s+and)', description)
                if book_match:
                    book = book_match.group(0).strip()
                    # Clean up the match to remove trailing words
                    book = re.sub(r'\s+(will\s+teach|and|was\s+an\s+instant).*$', '', book)
                else:
                    # Try finding patterns in quotes or after "his book" or "her book"
                    quote_match = re.search(r'(?:his|her|their)\s+(?:debut\s+)?book,?\s+([A-Z][A-Z\s:]+?)[\.,]', description)
                    if quote_match:
        ...
        if description:
            if "Central branch of the Austin Public Library" in description or "Austin Public Library" in description:
    ...
    def extract_book_club_events(self, html_content, url):
        ...
        for item in book_club_items:
            try:
                ...
                club_name_elem = item.find('div', class_='h1 smaller centered book-club')
                ...
                description_elem = item.find('div', class_='body-text center book-club w-richtext')
                ...
                full_club_name_match = re.search(r'The\s+([^.]+?Book\s+Club)', description_text)
                # Remove "The" prefix for consistency with test expectations
                if full_club_name_match:
                    full_club_name = full_club_name_match.group(1)  # Get the part without "The"
                else:
                ...
                book_info_match = re.search(r'selection:\s*([^b]+?)\s*by\s+([^.()]+?)(?:\s*\([^)]+\))?\.\s*Meets', description_text)
                ...
                if book_info_match:
                    ...
                    book_title = re.sub(r'^[A-Za-z]+/[A-Za-z]+\s+selection:\s*', '', book_title)
                    book_title = re.sub(r'^\w+\s+selection:\s*', '', book_title)
                    
                    # Remove any HTML artifacts or links
                    book_title = re.sub(r'<[^>]+>', '', book_title)
                    
                # If we didn't find book/author in the expected pattern, try link text
                if not book_title:
                    ...
                    if book_link:
                        ...
                        author_match = re.search(r'\s*by\s+([^.()]+?)(?:\s*\([^)]+\))?\.\s*Meets', remaining_text)
                        if author_match:
                            author = author_match.group(1).strip()
                
                # Extract meeting date and time
                # Look for patterns like "Meets Friday, June 27th at 7pm" or "Meets Wednesday, July 30th at 7pm"
                meeting_match = re.search(r'Meets\s+([^.]+?)\s+at\s+(\d+)(?::\d+)?\s*([ap]m)', description_text)
                ...
                # Look for patterns like "Hosted by [Title] [Name]" and extract just the name
                host_match = re.search(r'Hosted\s+by\s+([^.]+?)(?:\.|$)', description_text)
                host = None
                if host_match:
                    host_text = host_match.group(1).strip()
                    # Extract just the person's name (typically the last 1-2 words)
                    # Look for patterns like "First Light [title] Name" or "Name"
                    name_match = re.search(r'(?:First Light [^\.]+?\s+)([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)', host_text)
                    if name_match:
                ...
                # Find where host information ends and truncate there (before selection details)
                ...
                main_description = main_description.replace('\u2019', "'")  # Smart apostrophe to regular
                main_description = main_description.replace('\u201c', '"')  # Smart quote left to regular
                main_description = main_description.replace('\u201d', '"')  # Smart quote right to regular
