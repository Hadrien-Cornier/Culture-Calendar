...
class SummaryGenerator:
    ...
    def generate_summary(self, event: Dict, force_regenerate: bool = False) -> Optional[str]:
        """
        ...
        if not self._validate_event_data(event):
            print(f"  Insufficient data for summary generation: {event.get('title', 'Unknown')}")
            return None
        
        cache_key = f"{event.get('title', '').upper().strip()}_{event.get('type', 'unknown')}"
    ...
    def _validate_event_data(self, event: Dict) -> bool:
        ...
        has_basic_metadata = bool(event.get('director') or event.get('venue') or event.get('type'))
