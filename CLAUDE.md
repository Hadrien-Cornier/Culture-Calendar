...
- **Add a venue (live site) — config only, no Python**: add a `venues:` entry (display_name, address, classification policy, optional `extraction.batch_description`) AND a `web_llm_scrapers:` entry (base_url, venue_name, urls, fetch mode) in master_config.yaml. `src/scraper.py` builds one `WebLlmScraper` per registry entry and registers it for scraping automatically. `fetch:` is `http` (default), `playwright` (client-rendered listings), or `perplexity` (sites that 403 scrapers) — escalating is a one-line config flip. Smoke with `python update_website_data.py --test-week`. Prefer this over a new class.
- **Add a venue (bespoke class)**: only when the site exposes structured data worth parsing directly (JSON-LD, an API) or needs multi-step navigation. Extend `BaseScraper` in `src/scrapers/<venue>_scraper.py`; add config under `venues:`; register in `src/scrapers/__init__.py` + `MultiVenueScraper`; unit tests at `tests/test_<venue>_scraper_unit.py`.
...
This project is indexed by GitNexus as **Culture-Calendar** (3941 symbols, 9314 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
